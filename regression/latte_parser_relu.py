...
def write_latte_prediction(output, weights, y, problem_idx, relu, folder):
    """
    Polytope
    """
    d = len(weights)
    ...
    contents = {weight_indices[w] + 1: relu[w] for w in weights}
    contents[0] = relu["constant"]
    hlines += [create_line(d + 1, contents)]

    assert len(hlines) == m
    # write to file
    hlines = [f"{m} {d + 1}"] + [" ".join(str(round(entry * 10**PRECISION)) for entry in hline) for hline in hlines]
    latte_dir = os.path.join(folder, f"prediction_{problem_idx}")
    ...
    """
    d = len(weights)
    relu_coef = output['triangular']['center']['relu']
    monomials = []
    # constant one
    coefficient = Fraction(relu["constant"] * relu_coef)
    exp_vec = create_line(d, {})
    monomials.append((coefficient, exp_vec))

    # weight monomials
    for w in weights:
        coefficient = Fraction(relu[w] * relu_coef)
        exp_vec = create_line(d, {weight_indices[w]: 1})
        monomials.append((coefficient, exp_vec))
    ...
    """
    import csv
    gt = {"y": output["query"], "constant": output['triangular']['center']['constant']}
    with open(f'{folder}/gt_{problem_idx}.csv','w') as f:
        fieldnames = ['k', 'v']
...
if __name__ == '__main__':
    ...
    for INPUT_FILE in os.listdir(FOLDER):
        ...
        # # for test relu
        # if "relu" in INPUT_FILE:
        #     continue

        # if INPUT_FILE != "input_150.txt":
        #     continue
        ...
        for line in f.readlines():
            ...
            if layer == "weight":
                ...
                # current version works for one single relu
                relu = dict()
                i = 1
                while i < len(params):
                    relu[params[i]] = float(params[i + 1])
                    i += 2
            elif layer == "output":
        ...
        """
        # case relu <  0
        y_0 = {w: 0 for w in relu}
        y_0["constant"] = y["constant"]
        output_0 = copy.deepcopy(output)
        output_0["triangular"]["center"] = y_0
        weights_0 = copy.deepcopy(weights)
        write_latte_likelihood(output_0, weights_0, y_0, problem_idx, [relu, 0], folder=likelihood_folder, note="relu0_")
        # case relu >= 0
        w_relu = y["relu"]
        y_1 = {w: relu[w] * w_relu for w in relu}
        y_1["constant"] += y["constant"]
        output_1 = copy.deepcopy(output)
        output_1["triangular"]["center"] = y_1
        weights_1 = copy.deepcopy(weights)
        write_latte_likelihood(output_1, weights_1, y_1, problem_idx, [relu, 1], folder=likelihood_folder, note="relu1_")

        """
        prediction
        """
        write_latte_prediction(output, weights, y, problem_idx, relu, folder=prediction_folder)