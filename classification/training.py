...
# for wmi
def get_weights(model, model_name):
    if model_name == "VGG16":
        weights = list(model.classifier.parameters())
        np_w_2 = weights[-4].cpu().detach().numpy()
        np_b_2 = weights[-3].cpu().detach().numpy()
        np_w = weights[-2].cpu().detach().numpy()
        np_b = weights[-1].cpu().detach().numpy()
        return np_w, np_b, np_w_2, np_b_2
    elif model_name == "PreResNet164":
        weights = list(model.fc.parameters())
    elif model_name == "WideResNet28x10":
        weights = list(model.linear.parameters())
    np_w = weights[-2].cpu().detach().numpy()
    np_b = weights[-1].cpu().detach().numpy()
    return np_w, np_b
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_weights', type=int, default=90)
    ...
    print(f"train: {loaders['train'].__len__() * loaders['train'].batch_size}")
    print(f"test: {loaders['test'].__len__() * loaders['test'].batch_size}")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (args.swag and (epoch + 1) > args.swag_start) and args.cov_mat:
            train_res = utils.train_epoch(loaders['train'], model, criterion, optimizer, cuda=torch.cuda.is_available())
        else:
            train_res = utils.train_epoch(loaders['train'], model, criterion, optimizer, cuda=torch.cuda.is_available())
        ...
        if (epoch + 1) % args.save_freq == 0 and (epoch + 1) > args.swag_start:
            utils.save_checkpoint(
                args.dir,
                epoch + 1,
                state_dict=model.state_dict(),
            ...
            if args.swag and epoch == args.epochs - 1:
                utils.save_checkpoint(
                    args.dir,
                    epoch + 1,
                    name='swag',
        ...
        if epoch >= args.epochs - n_weights:
            if args.model == "VGG16":
                np_w, np_b, np_w_2, np_b_2 = get_weights(model, args.model)
                collect_weights_2.append(np_w_2)
                collect_bias_2.append(np_b_2)
            else:    
                np_w, np_b = get_weights(model, args.model)
            collect_weights.append(np_w)
            collect_bias.append(np_b)
    ...
    collect_weights = np.array(collect_weights)
    collect_bias = np.array(collect_bias)
    np.savez(os.path.join(args.dir, "wmi.npz"), weights=collect_weights, bias=collect_bias)

    if args.model == "VGG16":
        collect_weights_2 = np.array(collect_weights_2)
        collect_bias_2 = np.array(collect_bias_2)
        np.savez(os.path.join(args.dir, "wmi_2.npz"), weights=collect_weights_2, bias=collect_bias_2)
